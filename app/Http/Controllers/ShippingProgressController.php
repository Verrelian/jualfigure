...
class ShippingProgressController extends Controller
{
    ...
    {
        $shipping = Shipping::where('payment_id', $payment_id)->firstOrFail();
    ...
    public function nextStage($payment_id)
    {
        ...
        if ($shipping->stage_index < 4) {
            if ($diff >= 10) {
                ...
                return response()->json([
                    ...
                    'diff' => $diff,
                    'time_now' => now()->toDateTimeString(),
                    'last_updated_at' => $last->toDateTimeString(),
                ]);
            } else {
                return response()->json([
                    'message' => 'Too early',
        ...
        if ($shipping->stage_index === 4 && $diff >= 10) {
